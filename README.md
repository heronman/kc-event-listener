...
feedback.webhook[0].api-key=secret-1
...
```bash
export KCEL_FEEDBACK_WEBHOOK_0_API_KEY=prod-secret
```
...
- **Webhook**: `POST` to `url`, `Content-Type: application/json`, `X-Api-Key` header if `api-key`
  is set. Body is the full `Event` (type, time, realmId, clientId, userId, sessionId, ipAddress,
  error, details).
